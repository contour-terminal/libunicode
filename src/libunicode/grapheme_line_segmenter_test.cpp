...
class event_logger final: public unicode::grapheme_segmentation_listener
{
  public:
    ...
    void on_ascii(std::string_view text) noexcept { UNSCOPED_INFO(fmt::format("[event_logger] on_ascii: {}\n", text)); }
...
TEST_CASE("grapheme_line_segmenter.mixed.1")
{
    ...
    REQUIRE(std::holds_alternative<ascii_sequence>(recorder.records().at(0)));
    REQUIRE(std::holds_alternative<complex_unicode_sequence>(recorder.records().at(1)));
    REQUIRE(std::holds_alternative<ascii_sequence>(recorder.records().at(2)));

    REQUIRE(std::get<ascii_sequence>(recorder.records().at(0)).value == "0123456789{"sv);
    REQUIRE(std::get<complex_unicode_sequence>(recorder.records().at(1)).value == "\xE2\x94\x80"sv);
    REQUIRE(std::get<complex_unicode_sequence>(recorder.records().at(1)).width == 1);
    REQUIRE(std::get<ascii_sequence>(recorder.records().at(2)).value == "}ABCDEF"sv);
}
