...
namespace detail
{
    ...
    {
        constexpr void on_invalid(std::string_view) noexcept {}
        constexpr void on_ascii(std::string_view) noexcept {}
        constexpr void on_grapheme_cluster(std::string_view, unsigned) noexcept {}
    };
...
{
template <>
struct formatter<unicode::detail::State>: formatter<string_view>
{
    auto format(unicode::detail::State const value, auto& ctx) const
    {
        string_view name;
        switch (value)
        {
...
namespace detail
{
    ...
    LIBUNICODE_INLINE std::pair<StopCondition, char const*> process_only_ascii(char const* start, char const* end) noexcept
    {
        ...
        size_t constexpr SimdBitWidth = 128;
        size_t constexpr simd_size = SimdBitWidth / 8;
        ...
        -> detail::unicode_process_result
    {
        ...
        while (true)
        {
            if (input == end)
            {
                ...
                state.utf8DecodeNext = input;
                return make_scan_result(consumedWidths, state.currentClusterStart, StopCondition::EndOfInput);
            }
            ...
            if (!detail::is_complex(*input))
            {
                ...
                state.utf8DecodeNext = input;
                // We've reached a non-complex Unicode character (US-ASCII).
                ...
                if (feed_grapheme_cluster(state, currentCodepoint, input) == GraphemeClusterBoundary::Found)
                {
                    if (consumedWidths + state.currentClusterWidth <= maxWidth)
                    {
                        ...
                        if (consumedWidths == maxWidth)
                        {
                            ...
                            // We've reached the end of the line.
                            state.utf8DecodeNext = state.currentClusterStart;
                            state.currentCodepointStart = state.currentClusterStart;
            ...
            else if (std::holds_alternative<unicode::Invalid>(result))
            {
                ...
                if (consumedWidths + state.currentClusterWidth <= maxWidth)
                {
                    ...
                    // Invalid sequence resets hints usually? Or acts as basechar?
                    // Typically U+FFFD.
                    // We don't update lastCodepointHint here, code didn't do it before.
                }
                else
                {
                    // Currently scanned grapheme cluster won't fit. Revert to cluster start.
...
class grapheme_line_segmenter<OptionalEventListener>
{
  public:
    ...
    LIBUNICODE_INLINE auto process(unsigned maxWidth) noexcept -> grapheme_segmentation_result
    {
        ...
        while (true)
        {
            detail::State const state = detail::make_state(next(), end(), processedTotalWidth, maxWidth);
            ...
            switch (state)
            {
                ...
                case State::ComplexUnicode: {
                    ...
                    detail::unicode_process_result const chunk = detail::process_only_complex_unicode(
